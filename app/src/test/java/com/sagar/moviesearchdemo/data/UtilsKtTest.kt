...
class UtilsKtTest {
    ...
    fun `getApiResponse should return error response with 500 status code on IOException`() {
        ...
        assertEquals("Network error: Simulated network error", actualResponse.errorBody()?.string())
    }
    ...
    fun `getApiResponse should return error response when API call throws a RuntimeException`() {
        ...
        assertEquals(500, actualResponse.code())
        assertEquals("Runtime error: Unexpected exception", actualResponse.errorBody()?.string())
    }
}